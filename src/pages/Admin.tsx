...
const Admin = () => {
  ...
  const handleEdit = (product: Product) => {
    ...
    } as Partial<Product> & { features: string });
    setIsEditing(true);
  ...
  const handleSave = async () => {
    ...
    const featuresString = form.features as string | string[] | undefined;
    if (typeof featuresString === "string") {
      featuresArr = featuresString
        .split(/\r?\n/)
        .map((f) => f.trim())
        .filter(Boolean);
      ...
      <div className="container py-10">
        ...
        <div className="max-w-2xl mx-auto bg-card rounded-lg shadow p-6 mb-8">
          ...
          <textarea
            ...
            value={typeof form.features === 'string' ? form.features : Array.isArray(form.features) ? form.features.join('\n') : ''}
            onChange={handleInput}
            className="border rounded px-3 py-2 w-full mb-4"
            rows={4}
          />
